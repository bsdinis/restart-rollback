...
int sign_value(int64_t key, int64_t timestamp,
    ...
    EVP_MD_CTX* rsa_sign_ctx = EVP_MD_CTX_new();
    if (EVP_DigestSignInit(rsa_sign_ctx, nullptr, EVP_sha256(), nullptr,
                           g_rsa_key) <= 0) {
    ...
    if (EVP_DigestSignFinal(rsa_sign_ctx, nullptr, &signature_size) <= 0) {
        EVP_MD_CTX_free(rsa_sign_ctx);
        return -1;
    }
