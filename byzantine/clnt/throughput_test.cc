...
std::map<std::string, std::pair<cb_func, setup_func>> funcs_by_op = {
    {"get",
     {// if you need to use more intricate things for the arguments, you can add
      // global variables
      []() -> int64_t { return byzantine::get_cb(1); },
      []() {
          auto get_callbck =
              [](int64_t ticket, int64_t,
                 ...
                 int64_t, bool success) {
                  if (success) {
                      fprintf(
                          stdout,
                          "%lu, %ld, %ld, %ld, %ld, byzantine get, reply,\n",
                          now_usecs(), global_load, global_tick_duration,
                          g_curr_tick, ticket);
                  }
              };
          if (byzantine::get_set_cb(get_callbck) == -1) {
    ...
    {"put",
     {// if you need to use more intricate things for the arguments, you can add
      ...
      []() -> int64_t {
          ...
          value.fill(1);
          return byzantine::put_cb(3, value);
      },
      []() {
          auto put_callbck = [](int64_t ticket, bool, int64_t) {
              fprintf(stdout,
                      "%lu, %ld, %ld, %ld, %ld, byzantine put, reply,\n",
                      now_usecs(), global_load, global_tick_duration,
                      g_curr_tick, ticket);
          };
          if (byzantine::put_set_cb(put_callbck) == -1) {
...
void warmup(std::chrono::seconds duration) {
    ...
    while ((std::chrono::system_clock::now() - start) < duration) {
        ...
        while ((std::chrono::system_clock::now() - tick_start) <
               wtick_duration) {
            byzantine::poll();
            print_progress(byzantine::n_calls_concluded(),
                           byzantine::n_calls_issued());
        }
    }
    byzantine::wait_for();
}
...
void load_test(std::chrono::seconds duration) {
    ...
    auto const baseline_calls = byzantine::n_calls_concluded();
    auto const start = std::chrono::system_clock::now();
    ...
    while ((std::chrono::system_clock::now() - start) < duration) {
        ...
        for (size_t i = 0; i < n_ops; i++) {
            if (i % 100 < global_pct_read) {
                int64_t const ticket =
                    (funcs_by_op.at("get").first)();  // call lambda
                fprintf(stdout,
                        "%lu, %ld, %ld, %ld, %ld, byzantine get, request,\n",
                        now_usecs(), global_load, global_tick_duration,
                        g_curr_tick, ticket);
            } else {
                int64_t const ticket =
                    (funcs_by_op.at("put").first)();  // call lambda
                fprintf(stdout,
                        "%lu, %ld, %ld, %ld, %ld, byzantine put, request,\n",
                        now_usecs(), global_load, global_tick_duration,
                        g_curr_tick, ticket);
            }
        }
        ...
        while ((std::chrono::system_clock::now() - tick_start) <
               tick_duration) {
            print_progress(byzantine::n_calls_concluded() - baseline_calls,
                           byzantine::n_calls_issued() - baseline_calls);
            byzantine::poll();
        }
        g_curr_tick += 1;
    }
...
int main(int argc, char** argv) {
    ...
    if (global_config_path.empty()) {
        if (byzantine::init() != 0) KILL("failed to init connection to QP");
    } else if (byzantine::init(global_config_path.c_str()) != 0) {
