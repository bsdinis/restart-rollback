...
void pbft_handler(int sig_number);
...
int SGX_CDECL main(int argc, char *argv[]) {
    ...
    if (setup_signals(ignore_handler, 2, SIGPIPE, SIGCHLD) == -1)
        KILL("failed to setup signals");
    if (setup_signals(pbft_handler, 6, SIGSEGV, SIGFPE, SIGQUIT, SIGTSTP,
                      SIGURG, SIGXCPU) == -1)
        KILL("failed to setup signals");
    ...
    pbft_enclave_start(global_eid, conf, my_idx,
