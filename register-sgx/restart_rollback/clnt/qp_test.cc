...
int main(int argc, char** argv) {
    ...
    ASSERT_EQ(restart_rollback::close(true), 0, "close");
    INFO("finished test");
...
void test_get_put() {
    // sync test
    {
        ...
        std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE>
            get_value;
        EXPECT_EQ(restart_rollback::get((int64_t)1, get_value, get_timestamp),
                  true, "get");
        ...
        EXPECT_EQ(restart_rollback::put((int64_t)1, put_value, put_timestamp),
                  true, "put");
        ...
        put_value.fill(2);
        EXPECT_EQ(
            restart_rollback::put((int64_t)1, put_value, new_put_timestamp),
            true, "put");
    ...
    // async test
    {
        int64_t ticket = restart_rollback::get_async(2);
        if (restart_rollback::wait_for(ticket) ==
            restart_rollback::poll_state::ERR) {
            ERROR("failed to wait for get");
            return;
        }
        auto get_reply = restart_rollback::get_reply<std::tuple<
            int64_t,
            std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE>,
            int64_t, bool>>(ticket);
        ...
        std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE>
            put_value;
        put_value.fill(1);
        ticket = restart_rollback::put_async(2, put_value);
        if (restart_rollback::wait_for(ticket) ==
            restart_rollback::poll_state::ERR) {
        ...
        ticket = restart_rollback::get_async(2);
        if (restart_rollback::wait_for(ticket) ==
            restart_rollback::poll_state::ERR) {
            ERROR("failed to wait for get");
            return;
        }
        get_reply = restart_rollback::get_reply<std::tuple<
            int64_t,
            std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE>,
            int64_t, bool>>(ticket);
...
void test_proxy_get_put() {
    // sync test
    {
        ...
        std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE>
            get_value;
        EXPECT_EQ(
            restart_rollback::proxy_get((int64_t)3, get_value, get_timestamp),
            true, "proxy get");
        ...
        int64_t put_timestamp = -1;
        EXPECT_EQ(
            restart_rollback::proxy_put((int64_t)3, put_value, put_timestamp),
            true, "proxy put");
        ...
        int64_t new_put_timestamp = -1;
        put_value.fill(2);
        EXPECT_EQ(restart_rollback::proxy_put((int64_t)3, put_value,
                                              new_put_timestamp),
                  true, "proxy put");
    ...
    // async test
    {
        int64_t ticket = restart_rollback::proxy_get_async(4);
        if (restart_rollback::wait_for(ticket) ==
            restart_rollback::poll_state::ERR) {
            ERROR("failed to wait for get");
            return;
        ...
        std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE>
            put_value;
        put_value.fill(1);
        ticket = restart_rollback::proxy_put_async(4, put_value);
        if (restart_rollback::wait_for(ticket) ==
            restart_rollback::poll_state::ERR) {
            ERROR("failed to wait for put");
            return;
        ...
        ticket = restart_rollback::proxy_get_async(4);
        if (restart_rollback::wait_for(ticket) ==
            restart_rollback::poll_state::ERR) {
            ERROR("failed to wait for get");
            return;
...
void test_pipelining() {
    ...
    std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE> value;
    value.fill(5);
    ...
    for (int i = 0; i < 10; i++) {
        switch (i % 3) {
            case 0:
                ...
                break;
            case 1:
                put_tickets.emplace_back(restart_rollback::put_async(9, value));
                break;
            case 2:
            default:
    ...
    for (int64_t const ticket : get_tickets) {
        auto reply = restart_rollback::get_reply<std::tuple<
            int64_t,
            std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE>,
            int64_t, bool>>(ticket);
...
void test_cb() {
    ...
    int ping_n = 0;
    ASSERT_EQ(
        restart_rollback::get_set_cb(
            [&get_n](int64_t, int64_t,
                     std::array<uint8_t,
                                register_sgx::restart_rollback::REGISTER_SIZE>,
                     int64_t, bool) { get_n++; }),
        0, "get_set_cb");
    ASSERT_EQ(restart_rollback::put_set_cb(
                  [&put_n](int64_t, bool, int64_t) { put_n++; }),
              0, "put_set_cb");
    ASSERT_EQ(restart_rollback::ping_set_cb([&ping_n](int64_t) { ping_n++; }),
              0, "ping_set_cb");

    restart_rollback::get_cb((int64_t)1);
    ...
    std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE> value;
    value.fill(1);
    restart_rollback::put_cb((int64_t)4, value);
    restart_rollback::get_cb((int64_t)2);
    restart_rollback::get_cb((int64_t)5);
    restart_rollback::ping_cb();
    restart_rollback::put_cb((int64_t)6, value);
    ...
    ASSERT_EQ(restart_rollback::get_set_cb(
                  [](int64_t, int64_t,
                     std::array<uint8_t,
                                register_sgx::restart_rollback::REGISTER_SIZE>,
                     int64_t, bool) {}),
              0, "get_set_cb");
    ASSERT_EQ(restart_rollback::put_set_cb([](int64_t, bool, int64_t) {}), 0,
              "put_set_cb");
