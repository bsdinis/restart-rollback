...
int SGX_CDECL main(int argc, char *argv[]) {
    ...
    if (setup_signals(ignore_handler, 2, SIGPIPE, SIGCHLD) == -1)
        KILL("failed to setup signals");
    if (setup_signals(crash_handler, 6, SIGSEGV, SIGFPE, SIGQUIT, SIGTSTP,
                      SIGURG, SIGXCPU) == -1)
        KILL("failed to setup signals");
    ...
    restart_rollback_enclave_start(
        global_eid, conf, my_idx, create_mapping(mapping_filename), MAPPED_SIZE,
...
void shutdown_properly(int code, void *__) {
    ...
    LOG("shutdown restart_rollback properly");
}
