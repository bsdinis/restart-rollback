 ...
 *   get_reply(ticket) retrieves (and erases) the result of a certain
 * computation
 *
 * the callback API allows for callbacks to be registered to handle the results
 *   when a reply arrives whatever callback is registered *at that time* is
 * employed. by default does nothing
 */
 ...
 *
 * when a reply is ready, get_reply can be called (at most once per ticket) to
 * retrieve the result
 *
 */
 ...
 *
 * a callback for a function like `int f()` should have the following type
 * signature: `void f_callback(int64_t ticket, int res)`
 *
 * functions need to be called in a special manner
 * calling poll with no arguments advances all calls; when there are no more
 * outlasting calls poll returns NO_CALLS there is also an equivalent for
 * wait_for
 *
 */
