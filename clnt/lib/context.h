...
class get_call_ctx {
    public:
    get_call_ctx(int64_t ticket, int64_t key) : m_metadata_ticket(ticket), m_key(key) {}

    void set_untrusted_ticket(int64_t ticket) {
        m_untrusted_ticket = ticket;
    }

    void set_metadata(Metadata && metadata, int64_t policy_version, int64_t timestamp) {
        m_metadata = metadata;
    ...
    int64_t metadata_ticket() const {
        return m_metadata_ticket;
    }
    int64_t untrusted_ticket() const {
        return m_untrusted_ticket;
    }
...
class put_call_ctx {
    public:
    put_call_ctx(int64_t ticket, int64_t key, Metadata && metadata) : m_untrusted_ticket(ticket), m_key(key), m_metadata(metadata) {}
...
int get_call_tickets(int64_t teems_ticket, int64_t * metadata_ticket, int64_t * untrusted_ticket);
