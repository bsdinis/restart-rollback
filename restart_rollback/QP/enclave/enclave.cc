...
void restart_rollback_enclave_start(config_t conf, ssize_t my_idx,
                                    void *file_mapping, size_t mapping_size,
                                    void *measure_buffer, size_t buffer_size) {
    perf_rec = perf::perf_recorder(measure_buffer, buffer_size);
    setup::setup(&conf, my_idx, file_mapping, mapping_size);
    INFO("setup on %s:%d and %s:%d", conf.nodes[my_idx].addr,
         conf.nodes[my_idx].port, conf.nodes[my_idx].addr,
         conf.nodes[my_idx].port * 2);
    INFO("write quorum: %zu |\t read quorum: %zu + s",
         setup::write_quorum_size(), setup::read_quorum_size(0));
