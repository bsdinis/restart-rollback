...
int client_fast_get_handler(peer &p, int64_t ticket,
    ...
    auto result = paxos_sgx::restart_rollback::CreateMessage(
        builder, paxos_sgx::restart_rollback::MessageType_client_fast_get_resp, ticket,
        paxos_sgx::restart_rollback::BasicMessage_FastGetResult,
...
int client_operation_handler(peer &p, int64_t ticket,
    ...
    if (g_log.get_accepts(slot_n) >= paxos_sgx::restart_rollback::setup::quorum_size()) {
        g_log.accepted(slot_n);
    }
    while (g_log.get_accepts(slot_n) >=
               paxos_sgx::restart_rollback::setup::quorum_size() &&
    ...
    auto propose =
        paxos_sgx::restart_rollback::CreateReplicaPropose(builder, op_args, slot_n);
    auto message = paxos_sgx::restart_rollback::CreateMessage(
        builder, paxos_sgx::restart_rollback::MessageType_replica_propose, ticket,
        paxos_sgx::restart_rollback::BasicMessage_ReplicaPropose, propose.Union());
