...
int handle_client_message(peer &p) {
    ...
    while (p.buffer().size() > 0) {
        ...
        switch (request->type()) {
            case paxos_sgx::restart_rollback::MessageType_client_fast_get_req:
                ...
                break;
            case paxos_sgx::restart_rollback::MessageType_client_operation_req:
                ...
                break;
            case paxos_sgx::restart_rollback::MessageType_ping_req:
                client_ping_handler(p, request->ticket());
                perf_rec.add("ping");
                break;
            case paxos_sgx::restart_rollback::MessageType_reset_req:
                reset_handler(p, request->ticket());
                perf_rec.add("reset");
                break;
            case paxos_sgx::restart_rollback::MessageType_close_req:
    ...
    while (p.buffer().size() > 0) {
        ...
        switch (message->type()) {
            case paxos_sgx::restart_rollback::MessageType_replica_propose:
                ...
                break;
            case paxos_sgx::restart_rollback::MessageType_replica_accept:
