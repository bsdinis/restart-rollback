...
static sgx_status_t SGX_CDECL sgx_restart_rollback_enclave_start(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_restart_rollback_enclave_start_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_restart_rollback_enclave_start_t* ms = SGX_CAST(ms_restart_rollback_enclave_start_t*, pms);
	...
	restart_rollback_enclave_start(ms->ms_conf, ms->ms_my_idx, _tmp_mmaped_addr, ms->ms_addr_size, _tmp_measure_buffer, ms->ms_buffer_size);
...
SGX_EXTERNC const struct {
	...
	1,
	{
		{(void*)(uintptr_t)sgx_restart_rollback_enclave_start, 0, 0},
	}
};
...
sgx_status_t SGX_CDECL ocall_sgx_connect_block(int* retval, int* socket, const char* addr, uint16_t port)
{
	...
	if (status == SGX_SUCCESS) {
		if (retval) *retval = ms->ms_retval;
		if (socket) {
			if (memcpy_s((void*)socket, _len_socket, __tmp_socket, _len_socket)) {
				sgx_ocfree();
				return SGX_ERROR_UNEXPECTED;
			}
		}
...
sgx_status_t SGX_CDECL ocall_net_start_listen_socket(int* retval, const char* server_addr, uint16_t server_port)
{
	...
	if (ADD_ASSIGN_OVERFLOW(ocalloc_size, (server_addr != NULL) ? _len_server_addr : 0))
		return SGX_ERROR_INVALID_PARAMETER;
