...
std::map<std::string, std::pair<cb_func, setup_func>> funcs_by_op = {
    {"get",
     {// if you need to use more intricate things for the arguments, you can add
      // global variables
      []() -> int64_t { return restart_rollback::get_cb(1); },
      []() {
          auto get_callbck = [](int64_t ticket, int64_t,
                                std::array<uint8_t,
                                           register_sgx::restart_rollback::
                                               REGISTER_SIZE>,
                                int64_t, bool success) {
              if (success) {
                  fprintf(
                      stdout,
                      "%lu, %ld, %ld, %ld, %ld, restart_rollback get, reply,\n",
                      now_usecs(), global_load, global_tick_duration,
                      g_curr_tick, ticket);
              }
          };
          if (restart_rollback::get_set_cb(get_callbck) == -1) {
     ...
     {// if you need to use more intricate things for the arguments, you can add
      ...
      []() -> int64_t {
          std::array<uint8_t, register_sgx::restart_rollback::REGISTER_SIZE>
              value;
          value.fill(1);
          return restart_rollback::put_cb(3, value);
      },
      []() {
          auto put_callbck = [](int64_t ticket, bool, int64_t) {
              fprintf(stdout,
                      "%lu, %ld, %ld, %ld, %ld, restart_rollback put, reply,\n",
                      now_usecs(), global_load, global_tick_duration,
                      g_curr_tick, ticket);
          };
          if (restart_rollback::put_set_cb(put_callbck) == -1) {
    ...
    while ((std::chrono::system_clock::now() - start) < duration) {
        ...
        for (size_t i = 0; i < n_ops; i++) {
            if (i % 100 < global_pct_read) {
                ...
                fprintf(
                    stdout,
                    "%lu, %ld, %ld, %ld, %ld, restart_rollback get, request,\n",
                    now_usecs(), global_load, global_tick_duration, g_curr_tick,
                    ticket);
            } else {
                int64_t const ticket =
                    (funcs_by_op.at("put").first)();  // call lambda
                fprintf(
                    stdout,
                    "%lu, %ld, %ld, %ld, %ld, restart_rollback put, request,\n",
                    now_usecs(), global_load, global_tick_duration, g_curr_tick,
                    ticket);
            }
        }
