...
#include "restart_rollback_req_generated.h"
...
int handle_client_message(peer &p) {
    ...
    while (p.buffer().size() > 0) {
        ...
        auto request = paxos_sgx::restart_rollback::GetBasicRequest(p.buffer().data() +
                                                           sizeof(size_t));

        auto const begin = timer::now();
        switch (request->type()) {
            case paxos_sgx::restart_rollback::ReqType_sum:
                sum_handler(p, request->ticket(),
                            *request->args_as_SumArgs()->vec());
                perf_rec.add("sum", timer::elapsed_usec(begin));
                break;
            case paxos_sgx::restart_rollback::ReqType_ping:
                ping_handler(p, request->ticket());
                perf_rec.add("ping");
                break;
            case paxos_sgx::restart_rollback::ReqType_close:
