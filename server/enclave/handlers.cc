...
int handle_client_message(peer &p) {
    ...
    while (p.buffer().size() > 0) {
        ...
        auto request = teems::GetMessage(p.buffer().data() + sizeof(size_t));

        auto const begin = timer::now();
        switch (request->type()) {
            case teems::MessageType_get_req:
                get_handler(p, request->ticket(),
                            request->message_as_GetArgs());
                perf_rec.add("get", timer::elapsed_usec(begin));
                break;
            case teems::MessageType_get_timestamp_req:
                ...
                break;
            case teems::MessageType_put_req:
            ...
            case teems::MessageType_reset_req:
                ...
                perf_rec.add("reset");
                break;
            case teems::MessageType_close_req:
...
int handle_replica_message(peer &p, size_t idx) {
    ...
    while (p.buffer().size() > 0) {
        ...
        auto message = teems::GetMessage(p.buffer().data() + sizeof(size_t));
        auto const begin = timer::now();
        switch (message->type()) {
            case teems::MessageType_get_req:
...
int send_greeting(peer &p) {
    ...
    auto greeting = teems::CreateGreeting(builder, g_current_id);
    g_current_id += 1;

    auto message =
        teems::CreateMessage(builder, teems::MessageType_client_greeting, -1,
                             teems::BasicMessage_Greeting, greeting.Union());
...
int peer_new_connection(int const listen_socket, std::vector<peer> &list,
    ...
    if (idx == -1) {
        ...
        try {
            ...
            list.emplace_back(teems::setup::ssl_ctx(), true);
        } catch (std::bad_alloc &) {
        ...
        list.emplace(std::begin(list) + idx, teems::setup::ssl_ctx(), true);
    }
