...
int handle_client_message(peer &p) {
    ...
    while (p.buffer().size() > 0) {
        ...
        switch (request->type()) {
            case paxos_sgx::pbft::MessageType_client_fast_get_req:
                ...
                break;
            case paxos_sgx::pbft::MessageType_client_operation_req:
                ...
                break;
            case paxos_sgx::pbft::MessageType_ping_req:
                client_ping_handler(p, request->ticket());
                perf_rec.add("ping");
                break;
            case paxos_sgx::pbft::MessageType_reset_req:
                reset_handler(p, request->ticket());
                perf_rec.add("reset");
                break;
            case paxos_sgx::pbft::MessageType_close_req:
...
int handle_replica_message(peer &p) {
    ...
    while (p.buffer().size() > 0) {
        ...
        auto const begin = timer::now();
        switch (message->type()) {
            case paxos_sgx::pbft::MessageType_replica_pre_prepare:
                replica_pre_prepare_handler(
                    p, message->ticket(),
                    message->message_as_ReplicaPrePrepare());
                perf_rec.add("replica_pre_prepare", timer::elapsed_usec(begin));
                break;
            case paxos_sgx::pbft::MessageType_replica_prepare:
                replica_prepare_handler(p, message->ticket(),
                                        message->message_as_ReplicaPrepare());
                perf_rec.add("replica_prepare", timer::elapsed_usec(begin));
                break;
            case paxos_sgx::pbft::MessageType_replica_commit:
                replica_commit_handler(p, message->ticket(),
                                       message->message_as_ReplicaCommit());
                perf_rec.add("replica_commit", timer::elapsed_usec(begin));
                break;
            case paxos_sgx::pbft::MessageType_ping_req:
...
int peer_new_connection(int const listen_socket, std::vector<peer> &list,
    ...
    if (idx == -1) {
        ...
        list.emplace(std::begin(list) + idx, paxos_sgx::pbft::setup::ssl_ctx(),
                     true);
    }
